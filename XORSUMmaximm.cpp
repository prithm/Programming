...
int find_max_xor(TrieNode* root, int num, int ret, int level) {

	// cout << num << " " << ret << " " << level << endl;
	if(level == 0) return ret;
	int num_dig = num / (1 << (level-1));
	// long long int ret = 0;
	// cout << num_dig << endl;
	if(num_dig == 0) {
		if(root->right != NULL) ret = find_max_xor(root->right, num % (1 << (level-1)), ((ret << 1) + 1), level-1);
		else if(root->left != NULL) ret = find_max_xor(root->left, num % (1 << (level-1)), ((ret << 1) + 0), level-1);
	}
	else if(num_dig == 1) {
		if(root->left != NULL) ret = find_max_xor(root->left, num % (1 << (level-1)), ((ret << 1) + 1), level-1);
		else if(root->right != NULL) ret = find_max_xor(root->right, num % (1 << (level-1)), ((ret << 1) + 0), level-1);
	}
	return ret;
}
...
int main(){
	...
	while(t--){
		...
		for(int i=0; i<n; i++){
			...
			ans = max(ans, find_max_xor(root, pre, 0, max_level));
